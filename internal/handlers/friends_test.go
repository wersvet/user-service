...
func TestSendRequestInvalidBody(t *testing.T) {
	handler := NewFriendHandler(new(mocks.MockFriendRepository), services.NewUserService(new(mocks.MockAuthClient)))
...
func TestSendRequestPendingExists(t *testing.T) {
	...
	handler := NewFriendHandler(mockFriends, userSvc)
	router := setupFriendsRouter(handler)
...
func TestAcceptRequestSuccess(t *testing.T) {
	mockFriends := new(mocks.MockFriendRepository)
	handler := NewFriendHandler(mockFriends, services.NewUserService(new(mocks.MockAuthClient)))
	router := setupFriendsRouter(handler)
